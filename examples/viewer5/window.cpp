...
void Window::onUpdate() {
  ...
  m_viewMatrix = glm::lookAt(glm::vec3(0.0f, 0.0f, distanceFromOrigin),
...
void Window::onPaintUI() {
  ...
  // Create main window widget
  {
    ...
    // Textura
    {
      ...
      if (m_model.isUVMapped())
       comboItems.emplace_back("Default");

      ImGui::PushItemWidth(120);
     if (ImGui::BeginCombo("Textura", comboItems.at(m_mappingMode).c_str())) {
    for (auto const index : iter::range(comboItems.size())) {
        auto const isSelected{m_mappingMode == static_cast<int>(index)};
        if (ImGui::Selectable(comboItems.at(index).c_str(), isSelected)) {
            if (index == 0 ) {
               atualizaTexturaNormal = "maps/brick_base.jpg";
               atualizaTexturaDifuse = "maps/brick_normal.jpg"   ;
            } else if (index == 1){
    ...
    // Object Box
    {
      ...
      ImGui::PushItemWidth(120);
     if (ImGui::BeginCombo("Object", comboItems.at(m_mappingMode).c_str())) {
    for (auto const index : iter::range(comboItems.size())) {
        auto const isSelected{m_mappingMode == static_cast<int>(index)};
        if (ImGui::Selectable(comboItems.at(index).c_str(), isSelected)) {
            if (index == 0 ) {
